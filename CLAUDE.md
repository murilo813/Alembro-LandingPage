...
- `getWebPlans()` → `GET /web/plans`. Sem autenticação — catálogo de planos (tabela `planos` no backend), cada item com `slug`, `nome`, `descricao`, `precoMensal` (base, no caso do Personalizado), `empresasIncluidas`, `usuariosIncluidos` (por empresa), `precoEmpresaExtra`/`precoUsuarioExtra` (só preenchidos no Personalizado) e `recursos`. Chamado tanto no modo visitante quanto no modal "Alterar plano" (`pages/conta.html`, `loadFlowPlans()`, cacheado em `flowPlans` pra não rebuscar toda vez que o modal abre).
...
- `getWebInvoices(token)` → `GET /web/invoices`. Faturas já **pagas**, da mais recente pra mais antiga (máx. 24). Não inclui a próxima cobrança — essa é projetada no front a partir de `ativaAte` (ver "Fluxo no `pages/conta.html`" abaixo).
...
- **Faturas**: reais, via `GET /web/invoices` (`renderInvoices()`). Lista as pagas (data, valor, forma de pagamento, link do recibo da Asaas) e, no topo, **no máximo uma linha de próxima cobrança** — não vem do backend, é projetada a partir de `sub.ativaAte`, só aparece quando `sub.status === 'ativa'` e tem uma pill cinza "Prevista" (não "Paga") pra não passar por pagamento confirmado. Cancelar a assinatura remove essa linha (`renderInvoices()` roda de novo depois do cancelamento). `pagoEm` das faturas é data pura (`"2026-08-22"`, sem hora) — formatada na mão (`formatDataPura`), nunca com `new Date()`, que leria como meia-noite UTC e mostraria o dia anterior no fuso do Brasil. `urlRecibo` (o link do recibo, vindo da Asaas via nosso backend) passa por `urlReciboSegura()` (exige `https://` explícito — bloqueia um `javascript:` disfarçado) e `escapeAttr()` (escapa aspas/`<`/`>`) antes de entrar no `href` — é o único campo de fatura interpolado cru dentro de `innerHTML`; os outros ou vão por `textContent`/comparação com constante fixa, ou passam por formatador que só aceita o formato esperado.
