...
```
index.html, 404.html, delete.html, CNAME, robots.txt, sitemap.xml   (raiz — exigido pelo GitHub Pages, ver nota do delete.html abaixo)
pages/
  privacy.html    política de privacidade
  delete.html     solicitação de exclusão de conta (Alembro APP e STOCK)
  thanks.html     página de agradecimento pós-contato
  conta.html      painel de conta do Alembro FLOW (login/sessão/empresas/assinante/plano/faturas — tudo real, ver seção abaixo)
assets/
...
**Não renomeie nem mova `pages/delete.html` nem `pages/privacy.html`.** Essas URLs estão cadastradas fora deste repositório (App Store Connect / Google Play Console — link de exclusão de conta e política de privacidade exigidos por política das lojas) e em `sitemap.xml`. **Já aconteceu de verdade**: a reorganização de pastas de 21/08/2026 (`e5f618f`) moveu `delete.html` da raiz pra `pages/`, e ninguém atualizou o cadastro na Play Console — resultado, `https://alembro.com/delete` (a URL antiga, servida via resolução de extensão do Jekyll) virou 404 e a Play Console rejeitou o app com "Link para a Exclusão de Dados inválido" (achado em 24/08/2026). **`delete.html` na raiz hoje é justamente o stub de redirect** dessa correção (meta refresh + `location.replace` pra `pages/delete.html`, `noindex` + `rel="canonical"` pra não confundir SEO) — mantém a URL antiga viva pra qualquer lugar que ainda a referencie, mas a URL correta cadastrada na Play Console deveria ser `https://alembro.com/pages/delete.html`. Se algum dia precisar mesmo mover essas páginas de novo, deixe um redirect no caminho antigo, do mesmo jeito.
...
**Alembro FLOW tem login/sessão/empresas/assinatura/faturas 100% reais** (contra o `AlembroFLOWbackend`, ver seção abaixo) — **inclusive cobrança de verdade**: o fluxo de assinar um plano, pagar na Asaas e o webhook liberar o acesso já foi confirmado ponta a ponta (22/08/2026). `buildAccountSession()` é o ponto de junção: pega a identidade real (nome/e-mail/token) do login e monta a sessão salva; empresas, status de assinante/trial, assinatura e faturas são preenchidos à parte, todos com dados reais, logo depois (ver "Sessão" abaixo).
...
- `calculateCustomPlanPrice(plan, companies, users)` / `formatBRL(value)` — fórmula do plano Personalizado: preço base do `plan` (empresas/usuários incluídos, vindos de `GET /web/plans`) + valor por empresa extra + valor por usuário extra, sem os descontos de pacote que Multi/Equipe têm. Todos os planos são mensais. **`users` é por empresa, não um total** (o próprio texto dos planos já diz "2 usuários por empresa") — o valor do usuário extra multiplica pelo número de empresas (5 empresas/5 usuários por empresa = 4 empresas extras + 3 usuários extras **em cada uma das 5**, não 3 ao todo). Tem que bater com `calculate_price_cents` no backend, que é quem decide o valor cobrado de verdade — aqui é só a prévia na tela. Chamava `calcularPrecoPersonalizado` e lia campos tipo `plan.precoMensal` até 24/08/2026 — nome de código e chave de JSON são em inglês agora (`plan.monthlyPrice` etc.), igual ao padrão que `products`/`clients` já usavam no backend; só texto que o usuário lê de verdade (nome do plano, descrição) continua português, como dado.
...
- `getWebPlans()` → `GET /web/plans`. Sem autenticação — catálogo de planos (tabela `planos` no backend), cada item com `slug`, `name`, `description`, `monthlyPrice` (base, no caso do Personalizado), `includedCompanies`, `includedUsers` (por empresa), `extraCompanyPrice`/`extraUserPrice` (só preenchidos no Personalizado) e `features`. Chamado tanto no modo visitante quanto no modal "Alterar plano" (`pages/conta.html`, `loadFlowPlans()`, cacheado em `flowPlans` pra não rebuscar toda vez que o modal abre).
- `getWebSubscription(token)` → `GET /web/subscription`. Assinatura atual ou `null`. Continua vindo preenchida depois de cancelar, enquanto `activeUntil` estiver no futuro.
- `getWebInvoices(token)` → `GET /web/invoices`. Faturas já **pagas**, da mais recente pra mais antiga (máx. 24). Não inclui a próxima cobrança — essa é projetada no front a partir de `activeUntil` (ver "Fluxo no `pages/conta.html`" abaixo).
- `subscribeToPlan(token, planSlug, { empresas, usuarios })` → `POST /web/subscribe`. Devolve `{ checkoutUrl }` pra redirecionar o usuário. **Não manda valor nenhum** — ver abaixo. `usuarios` é por empresa, mesmo detalhe da calculadora acima.
...
1. **O front nunca manda valor em reais.** `subscribeToPlan()` envia só a intenção (`planSlug` + quantidades no Personalizado); o backend recalcula o preço a partir da tabela `planos`. O cálculo que existe aqui (`calculateCustomPlanPrice`) é **só pra exibir** o preço ao vivo na calculadora — não é a fonte de verdade de nada.
2. **O acesso só é liberado pelo backend, via webhook do gateway.** A volta do checkout (`?assinatura=processando`) é apenas uma dica de UI: qualquer um pode digitar essa URL. O painel entra em "Confirmando seu pagamento..." e fica revalidando `GET /web/session` (`waitForPaymentConfirmation()`, 3s, teto de 2min) até `subscriber` virar `true` de verdade. Nunca marque `user.subscriber = true` no cliente — foi exatamente o que a versão mock fazia e que precisou sair.

**Trocar de plano cancela a assinatura atual na hora, no backend, mesmo que o usuário desista.** `subscribeToPlan()` já cancela a assinatura ativa (na Asaas e no banco) antes de criar o checkout novo — fechar a aba sem terminar o pagamento não desfaz isso. Achado real (22/08/2026, testando o fluxo): selecionar um plano novo e fechar a tela deixou a assinatura anterior "cancelada" sem nenhum aviso — o acesso continuou (garantido até o fim do período já pago), mas a renovação automática já tinha sido desligada de verdade. Por isso o listener de clique em `.plan-select-btn` (`pages/conta.html`) mostra um `confirm()` antes de chamar `subscribeToPlan()`, só quando `user.subscription.status === 'ativa'` (primeira assinatura, saindo do trial, não tem esse risco — não existe nada pra cancelar).
...
- **Faturas**: reais, via `GET /web/invoices` (`renderInvoices()`). Lista as pagas (data, valor, forma de pagamento, link do recibo da Asaas) e, no topo, **no máximo uma linha de próxima cobrança** — não vem do backend, é projetada a partir de `sub.activeUntil`, só aparece quando `sub.status === 'ativa'` e tem uma pill cinza "Prevista" (não "Paga") pra não passar por pagamento confirmado. Cancelar a assinatura remove essa linha (`renderInvoices()` roda de novo depois do cancelamento). `paidOn` das faturas é data pura (`"2026-08-22"`, sem hora) — formatada na mão (`formatPlainDate`), nunca com `new Date()`, que leria como meia-noite UTC e mostraria o dia anterior no fuso do Brasil. `receiptUrl` (o link do recibo, vindo da Asaas via nosso backend) passa por `safeReceiptUrl()` (exige `https://` explícito — bloqueia um `javascript:` disfarçado) e `escapeAttr()` (escapa aspas/`<`/`>`) antes de entrar no `href` — é o único campo de fatura interpolado cru dentro de `innerHTML`; os outros ou vão por `textContent`/comparação com constante fixa, ou passam por formatador que só aceita o formato esperado.
- Modal "Alterar plano"/"Assinar" reaproveita a mesma função `planCardHTML()` usada na tela de visitante, em modo `'select'` (mostra botão "Selecionar" ou badge "Plano atual", só quando `user.subscriber` já é `true`). O card Personalizado tem inputs de texto sanitizados (só dígitos, sem o spinner feio do `input[type=number]`) que recalculam o preço ao vivo — o label do segundo input é "Usuários por empresa", não "Usuários" (ver a regra "por empresa" na seção "Assinatura" acima). Selecionar um plano diferente do atual, com assinatura já ativa, mostra o `confirm()` de aviso antes de chamar `subscribeToPlan()`.
- **Card "Empresas" mostra uso, não detalhes, e é real.** Vem de `GET /web/companies` — só empresas onde o usuário é `OWNER` (as que ele é admin/usuário comum não contam pro limite dele nem aparecem aqui). Decisão deliberada de UI: nada de CNPJ, nome completo em destaque ou papel/role — só uma barra "X de Y empresas utilizadas" (`user.companiesLimit`, espelha `usuarios.limite_empresas`) e, por empresa, uma barra "X de Y usuários" (`company.usersUsed`/`company.usersLimit`, espelha `empresas.limite_usuarios`). **Esses dois limites agora refletem o plano de verdade** — até 22/08/2026 o backend nunca escrevia neles ao ativar uma assinatura, então o card ficava travado no valor antigo mesmo o cliente pagando por mais (ver `CLAUDE.md` do backend). Não crie/edite empresa pelo site — isso também é só no app.
...
### Rotas do site — conectadas (`/web/login`, `/web/session`, `/web/companies`, `/web/update_user`, `/web/plans`, `/web/subscribe`, `/web/subscription`, `/web/cancel_subscription`, `/web/invoices`)
...
- `GET /web/invoices` retorna as faturas **pagas** (tabela `faturas`, até 24, mais recente primeiro) — gravadas pelo backend a cada `PAYMENT_CONFIRMED`, na mesma transação que libera o acesso.
...
- `planos`: `slug` (chave usada pelo front, ex. `'personalizado'` — não confundir com o `id` numérico, que a UI não usa), `nome`, `descricao`, `preco_mensal` (no Personalizado é a *base* do cálculo, não "sob consulta"), `empresas_incluidas`/`usuarios_incluidos` (**por empresa**, não total), `preco_empresa_extra`/`preco_usuario_extra` (só no Personalizado), `recursos` (`TEXT[]`, bullets fixos — os bullets calculados do Personalizado, tipo "+ R$25 por empresa adicional", são montados no front a partir dos preços, não ficam salvos como texto), `ativo`, `ordem_exibicao`.
- `assinaturas` guarda o estado real; `usuarios` ganhou `id_plano`, `plano_empresas_contratadas`/`plano_usuarios_contratados` (informativo — o que foi contratado), `plano_atualizado_em`, `assinatura_ate` **e** `limite_empresas` (esse último já existia, mas agora é escrito pela ativação da assinatura — é o que `/web/companies` e o app realmente aplicam como teto). `empresas.limite_usuarios` recebe o mesmo tratamento.
- `faturas`: uma linha por cobrança confirmada — é de onde `GET /web/invoices` lê.
...
O fluxo de assinatura foi testado ponta a ponta pela primeira vez: assinar um plano, pagar com cartão de teste na tela da Asaas, o webhook confirmar e o acesso liberar de verdade — inclusive troca de plano (Solo → Personalizado). Esse teste expôs (e já corrigiu, do lado do backend) três bugs: `assinatura_ate` avançando dois meses por cobrança em vez de um, os limites de empresa/usuário nunca sendo escritos ao ativar o plano, e o cálculo de usuário extra do Personalizado ignorando que é "por empresa". Detalhes de cada um no `CLAUDE.md` do backend.
...
- **Nomes de função/variável em inglês; texto em português.** Comentários, mensagens de erro, commits e qualquer string que carregue dado real (nome de plano, valor, texto que vem de resposta de API) ficam em português — é o que o usuário vê. Identificador de código (função, variável) é em inglês, mesmo quando o valor que ele guarda é português (ex: `formatDate(iso)`, não `formatarData`). Exceção: campo tirado do banco pra uso só local dentro da própria função pode ser aliasado direto pro inglês na hora de ler (sem precisar de duas camadas de tradução).
